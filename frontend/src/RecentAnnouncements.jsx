...
export default function RecentAnnouncements({ onSeeMore }) {
  const { t } = useI18n();
  const { user } = useAuth();
  ...
  // When a sale is selected, fetch its public detail with editions
  useEffect(() => {
  ...
  return (
    <section className="announcements">
      ...
      {/* See more + Suggest prompt */}
      <div className="announcements-footer">
        {onSeeMore && sales.length > 0 && (
          <button className="announcements-see-more" onClick={onSeeMore}>
            {t("announcements.seeMore")}
          </button>
        )}
        <div className="announcements-suggest">
                  ...
                  {selected.basePrice && (
                    <div className="ann-modal-meta-row">
                      ...
                      <span className="ann-modal-meta-value">
                        {selected.basePrice} {selected.currency || ""}
                      </span>
                    </div>
                  )}
                </div>
