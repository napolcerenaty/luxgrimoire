...
function StatCard({ label, value, sub, icon: Icon, accent, color }: {
  ...
  return (
    <div className={`rounded-2xl p-5 border flex flex-col gap-2 ${accent ? 'bg-brand-950/30 border-brand-700/40' : 'bg-navy-900 border-navy-800'}`}>
      <div className="flex items-center gap-2">
        <Icon size={14} className={color ?? (accent ? 'text-brand-400' : 'text-navy-500')} />
        <span className="text-xs uppercase tracking-wider text-navy-500">{label}</span>
      </div>
      <p className={`text-base sm:text-xl font-serif font-bold leading-tight ${color ?? (accent ? 'text-brand-400' : 'text-navy-100')}`}>{value}</p>
...
function TabLoading() {
  return <div className="text-center py-20 text-navy-500 animate-pulse">Loading…</div>
}
      ...
      {data.map((d, i) => {
        ...
        return (
          <div key={i} className="relative flex-1 group" style={{ height: TOTAL_H }}>
            {d.amount > 0 && (
              <div
                className="absolute z-10 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity bg-navy-800 border border-navy-700 rounded px-2 py-1 text-[10px] text-navy-200 whitespace-nowrap"
            ...
            />
            <span
              className="absolute left-0 right-0 text-center text-[9px] text-navy-600 bottom-0"
      ...
      {data.map((d, i) => {
        ...
        return (
          <div key={i} className="relative flex-1 group" style={{ height: TOTAL_H }}>
            ...
            />
            <span
              className="absolute left-0 right-0 text-center text-[9px] text-navy-500 bottom-0"
              style={{ height: LABEL_H, lineHeight: LABEL_H + 'px' }}
            >
              {d.year}
            </span>
    ...
    <div className="space-y-1">
      <div className="flex justify-between text-xs">
        ...
        <span className="text-navy-300 font-medium">
          {fmt(amount, currency)} <span className="text-navy-600">({pct.toFixed(1)}%)</span>
        </span>
      </div>
  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-xs">
        <span className="text-navy-400">{label}</span>
        <span className="text-navy-300 font-medium">
          {count} <span className="text-navy-600">({pct.toFixed(1)}%)</span>
        </span>
      </div>
      <div className="h-1.5 bg-navy-800 rounded-full overflow-hidden">
        <div className="h-full rounded-full transition-all duration-500" style={{ width: `${pct}%`, background: color }} />
        ...
        return (
          <div key={i} className="relative flex-1 group" style={{ height: TOTAL_H }}>
            {(d.amount > 0 || saleAmt > 0) && (
              <div
                className="absolute z-10 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity bg-navy-800 border border-navy-700 rounded px-2 py-1 text-[10px] whitespace-nowrap space-y-0.5"
    ...
    <div className="flex items-center gap-3 justify-between flex-wrap pt-1">
      <div className="flex items-center gap-2">
        <button
          ...
          disabled={!prev}
          className="p-1.5 rounded-lg border border-navy-700 text-navy-400 hover:text-navy-100 hover:border-navy-500 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
        >
          <ChevronLeft size={14} />
        </button>
        <span className="text-lg font-semibold text-navy-200 min-w-[4ch] text-center">{selected}</span>
        <button
          ...
          <button
            ...
            className={`px-3 py-1 rounded-lg text-sm border transition-colors ${
              y === selected
                ...
                : 'border-navy-700 text-navy-500 hover:border-navy-500 hover:text-navy-300'
            }`}
          >
            {y}
          </button>
      ...
      {allYears.map((year) => {
        ...
        return (
          <div key={year} className="relative flex-1 group" style={{ height: TOTAL_H }}>
            ...
            <span className="absolute left-0 right-0 text-center text-[9px] text-navy-500 bottom-0" style={{ height: LABEL_H, lineHeight: LABEL_H + 'px' }}>
              {year}
            </span>
          </div>
        )
...
function MonthlyPLBarChart({ data, currency }: { data: Array<{ month: string; pl: number }>; currency: string }) {
  ...
  return (
    <div className="relative w-full" style={{ height: TOTAL_H + 4 }}>
      {/* zero line */}
      <div className="absolute left-0 right-0 border-t border-navy-700/60" style={{ top: ZERO_Y }} />
        ...
        {data.map((d, i) => {
          ...
          return (
            <div key={i} className="relative flex-1 group" style={{ height: TOTAL_H }}>
              ...
              />
              <span className="absolute left-0 right-0 text-center text-[9px] text-navy-600 bottom-0" style={{ height: LABEL_H, lineHeight: LABEL_H + 'px' }}>
                {monthName}
              </span>
            </div>
          )
        ...
        {BUCKETS.map((b, i) => {
          ...
          return (
            <div key={i} className="relative flex-1 group" style={{ height: BAR_MAX_H + 20 }}>
              {counts[i] > 0 && (
                <div
                  className="absolute z-10 pointer-events-none opacity-0 group-hover:opacity-100 bg-navy-800 border border-navy-700 rounded px-2 py-1 text-[10px] text-navy-200 whitespace-nowrap transition-opacity"
              ...
              {counts[i] > 0 && (
                <div className="absolute w-full text-center text-[9px] text-navy-300 font-medium" style={{ bottom: 20 + h + 2, fontSize: 8 }}>
                  {counts[i]}
                </div>
              )}
              <div className="absolute left-0.5 right-0.5 rounded-t-sm" style={{ bottom: 20, height: h, background: b.color + 'bb' }} />
              <span className="absolute left-0 right-0 text-center text-[8px] text-navy-500 bottom-0 leading-tight" style={{ height: 18 }}>
                {b.label}
              </span>
            </div>
          )
      ...
      {data.slice(0, 10).map((c, i) => {
        ...
        return (
          <div key={i} className="space-y-0.5">
            <div className="flex justify-between text-xs">
              <span className="text-navy-400 truncate max-w-[55%]">{c.name}</span>
              <span className={`font-medium ml-2 shrink-0 ${isProfit ? 'text-emerald-400' : 'text-red-400'}`}>
                {isProfit ? '+' : ''}{fmt(c.pl, currency)}
                <span className="text-navy-600 ml-1">({c.count} sold)</span>
              </span>
            </div>
            <div className="h-1.5 bg-navy-800 rounded-full overflow-hidden">
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-serif font-bold text-navy-100">Statistics</h1>
          <p className="text-navy-400 text-sm mt-1">Your collection &amp; spending analytics</p>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          <span className="text-xs text-navy-500 uppercase tracking-wider">Display in</span>
          <select
            value={currency}
            onChange={e => { setCurrency(e.target.value) }}
            className="bg-navy-900 border border-navy-700 text-navy-100 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-brand-400"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 border border-navy-700 text-navy-400 hover:text-navy-200 hover:border-navy-500 px-3 py-2 rounded-xl text-sm transition-colors"
          >
            <Settings size={13} />
      ...
      {/* Disclaimer */}
      <div className="flex items-start gap-2 text-xs text-navy-600 leading-relaxed">
      ...
      {/* Tab bar */}
      <div className="flex flex-wrap gap-1.5 mb-4 p-1 bg-navy-900/60 border border-navy-800 rounded-xl">
        {visibleTabs.map(tab => {
          const tabIsStale = (
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 text-sm font-medium transition-colors rounded-lg relative ${
                activeTab === tab.id
                  ? 'bg-navy-800 text-brand-400 border border-navy-700'
      ...
      {settingsOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy-950/80 backdrop-blur-sm">
          <div className="bg-navy-900 border border-navy-800 rounded-2xl w-full max-w-sm overflow-hidden">
      ...
      {loadedTabs.has('collection') && (
        <div className={activeTab !== 'collection' ? 'hidden' : 'space-y-6'}>
          {collLoading ? <TabLoading /> : !collection ? (
            <div className="text-center py-20 text-navy-500">No data yet.</div>
          ) : (
            <>
              ...
              {collection.byCompanyAll.length > 0 && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  <div className="bg-navy-900 border border-navy-800 rounded-2xl overflow-hidden">
                    <div className="flex items-center gap-2 px-6 py-4 border-b border-navy-800">
                      <Layers size={14} className="text-brand-400" />
                      <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Books by Company</h2>
                      <span className="ml-auto text-[10px] text-navy-600">{collection.byCompanyAll.length} companies</span>
                    </div>
                    <div className="divide-y divide-navy-800/50">
                      {collection.byCompanyAll.slice(0, 8).map((c, i) => {
                        ...
                        return (
                          <div key={c.slug} className="px-5 py-3 hover:bg-navy-800/30 transition-colors">
                            <div className="flex items-center gap-3 mb-1.5">
                              <span className="text-xs text-navy-600 w-4 text-right font-mono shrink-0">{i + 1}</span>
                              <span className="flex-1 text-sm text-navy-200 truncate">{c.name}</span>
                              <span className="text-sm font-semibold text-indigo-400 shrink-0">{c.books}</span>
                              <span className="text-xs text-navy-600 shrink-0">book{c.books !== 1 ? 's' : ''}</span>
                            </div>
                            <div className="ml-7 h-1 bg-navy-800 rounded-full overflow-hidden">
                  ...
                  <div className="bg-navy-900 border border-navy-800 rounded-2xl p-6 space-y-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Acquisition Sources</h2>
                    </div>
                    {(() => {
                    ...
                    {collection.bySubscriptionAll.length > 0 && (
                      <div className="pt-3 border-t border-navy-800 space-y-2">
                        <p className="text-xs uppercase tracking-wider text-navy-500">By Subscription</p>
                        {collection.bySubscriptionAll.slice(0, 5).map((s) => {
                          ...
                          return (
                            <div key={s.slug} className="space-y-1">
                              <div className="flex justify-between text-xs">
                                <span className="text-navy-400 truncate">{s.name}</span>
                                <span className="text-navy-300 font-medium ml-2 shrink-0">{s.books}</span>
                              </div>
                  ...
                  {(collection.firstHandCount > 0 || collection.secondHandCount > 0) && (
                    <div className="bg-navy-900 border border-navy-800 rounded-2xl p-6 space-y-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Market Source</h2>
                      </div>
                      {(() => {
          ...
          {spLoading ? <TabLoading /> : !spending ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="lg:col-span-2 bg-navy-900 border border-navy-800 rounded-2xl p-6 space-y-5">
                  <div className="flex items-center gap-2">
                    <BarChart2 size={14} className="text-brand-400" />
                    {effectiveSettings.sales && spending.salesByMonth.some(m => m.amount > 0) ? (
                      <>
                        <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Spending vs Sales — {selectedYear}</h2>
                      ...
                      <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Spending — {selectedYear}</h2>
                    )}
                  </div>
                  {spending.byMonth.length === 0 ? (
                    <p className="text-navy-600 text-sm text-center py-8">No data for {selectedYear}</p>
                  ) : effectiveSettings.sales && spending.salesByMonth.some(m => m.amount > 0) ? (
                    ...
                    <>
                      <div className="flex items-center justify-between pt-2 border-t border-navy-800/60">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-semibold text-navy-400 uppercase tracking-wider">Books — {selectedYear}</span>
                        </div>
                ...
                <div className="bg-navy-900 border border-navy-800 rounded-2xl p-6 space-y-5">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">By Year</h2>
                  </div>
                  ...
                  {spending.byYearBooks && spending.byYearBooks.length > 0 && (
                    <>
                      <div className="flex items-center gap-2 pt-2 border-t border-navy-800/60">
                        <Library size={12} className="text-indigo-400" />
                        <span className="text-xs font-semibold text-navy-400 uppercase tracking-wider">Books Acquired</span>
                      </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="bg-navy-900 border border-navy-800 rounded-2xl p-6 space-y-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Cost Breakdown</h2>
                  </div>
                  ...
                  {savings > 0 && (
                    <div className="pt-2 border-t border-navy-800 space-y-1">
                      ...
                      <div className="flex justify-between text-xs font-semibold">
                        <span className="text-navy-300">Net total</span>
                ...
                <div className="bg-navy-900 border border-navy-800 rounded-2xl overflow-hidden">
                  <div className="flex items-center gap-2 px-6 py-4 border-b border-navy-800">
                    ...
                    <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Spending by Subscription</h2>
                  </div>
                  {spending.bySubscription.length === 0 ? (
                    <p className="text-navy-600 text-sm text-center py-8">No subscription data</p>
                  ) : (
                    <div className="divide-y divide-navy-800/50">
                      {spending.bySubscription.map((s, i) => {
                        ...
                        return (
                          <div key={s.slug} className="px-5 py-3.5 hover:bg-navy-800/30 transition-colors">
                            <div className="flex items-center gap-3 mb-1.5">
                              <span className="text-xs text-navy-600 w-4 text-right font-mono shrink-0">{i + 1}</span>
                              <span className="flex-1 text-sm font-medium text-navy-200 truncate">{s.name}</span>
                              <span className="text-sm font-semibold text-brand-400 shrink-0">{fmt(s.amount, currency)}</span>
                              <span className="text-xs text-navy-600 shrink-0">· {s.books}b</span>
                              <span className="text-[10px] text-navy-500 w-8 text-right shrink-0">{pct.toFixed(0)}%</span>
                            </div>
                            <div className="ml-7 h-1 bg-navy-800 rounded-full overflow-hidden">
              ...
              {spending.byCompany.length > 0 && (
                <div className="bg-navy-900 border border-navy-800 rounded-2xl overflow-hidden">
                  <div className="flex items-center gap-2 px-6 py-4 border-b border-navy-800">
                    <Tag size={14} className="text-brand-400" />
                    <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Spending by Company</h2>
                    <span className="ml-auto text-[10px] text-navy-600 font-medium">{spending.byCompany.length} companies</span>
                  </div>
                  <div className="divide-y divide-navy-800/50">
                    {spending.byCompany.map((c, i) => {
                      ...
                      return (
                        <div key={c.slug} className="px-6 py-4 hover:bg-navy-800/30 transition-colors">
                          <div className="flex items-center gap-4 mb-2">
                            <span className="text-xs text-navy-600 w-5 text-right font-mono shrink-0">{i + 1}</span>
                            <span className="flex-1 text-sm font-medium text-navy-200 truncate">{c.name}</span>
                            <span className="text-sm font-semibold text-brand-400 shrink-0">{fmt(c.amount, currency)}</span>
                            <span className="text-xs text-navy-600 shrink-0">· {c.books} book{c.books !== 1 ? 's' : ''}</span>
                            <span className="text-[11px] text-navy-500 w-10 text-right shrink-0">{pct.toFixed(1)}%</span>
                          </div>
                          <div className="ml-9 h-1.5 bg-navy-800 rounded-full overflow-hidden">
              ...
              {spending.topExpensive.length > 0 && (
                <div className="bg-navy-900 border border-navy-800 rounded-2xl overflow-hidden">
                  <div className="flex items-center gap-2 px-6 py-4 border-b border-navy-800">
                    ...
                    <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Top 10 Most Expensive</h2>
                  </div>
                  <div className="divide-y divide-navy-800/50">
                    {spending.topExpensive.map((book, i) => (
                      <div key={i} className="flex items-center gap-4 px-6 py-3 hover:bg-navy-800/30 transition-colors">
                        <span className="text-xs text-navy-600 w-5 text-right font-mono">{i + 1}</span>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-navy-100 font-medium truncate">{book.title}</p>
                          <p className="text-xs text-navy-500 truncate">{book.author}</p>
                        </div>
                        <div className="text-right shrink-0">
                          <p className="text-sm font-semibold text-brand-400">{fmt(book.amount, currency)}</p>
                          <p className="text-xs text-navy-600">{book.date}</p>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      {loadedTabs.has('sales') && (
        <div className={activeTab !== 'sales' ? 'hidden' : 'space-y-6'}>
          {salesLoading ? <TabLoading /> : !sales ? (
            ...
            <div className="text-center py-20 text-navy-500">No sales data yet.</div>
          ) : (
            <>
              {/* Sales totals */}
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                ...
                {sales.totalSalesProfit != null && (
                  <div className={`rounded-2xl p-5 border flex flex-col gap-2 ${sales.totalSalesProfit >= 0 ? 'bg-emerald-950/20 border-emerald-700/30' : 'bg-red-950/20 border-red-700/30'}`}>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs uppercase tracking-wider text-navy-500">Net P&amp;L</span>
                    </div>
                    ...
                    <p className="text-xs text-navy-500">revenue − purchase cost</p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="lg:col-span-2 bg-navy-900 border border-navy-800 rounded-2xl p-6">
                  <div className="flex items-center gap-2 mb-4">
                    <BarChart2 size={14} className="text-green-400" />
                    <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Sales Revenue — {salesYear}</h2>
                  </div>
                  {sales.salesByMonth.length === 0 ? (
                    <p className="text-navy-600 text-sm text-center py-8">No sales data for {salesYear}</p>
                  ) : (
                    <MonthBarChart
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="bg-navy-900 border border-navy-800 rounded-2xl p-6">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Sales by Platform</h2>
                  </div>
                    ...
                    <div className="space-y-3">
                      {sales.salesByPlatform.map((p) => (
                        <div key={p.platform} className="space-y-1">
                          <div className="flex justify-between text-xs">
                            <span className="text-navy-300 font-medium capitalize">{p.platform}</span>
                            <span className="text-navy-400">
                              {fmt(p.amount, currency)} <span className="text-navy-600">· {p.count} book{p.count !== 1 ? 's' : ''}</span>
                            </span>
                          </div>
                          <div className="h-1.5 bg-navy-800 rounded-full overflow-hidden">
                            <div
                ...
                {sales.salesByCompany.length > 0 && (
                  <div className="bg-navy-900 border border-navy-800 rounded-2xl overflow-hidden">
                    <div className="flex items-center gap-2 px-6 py-4 border-b border-navy-800">
                      <Tag size={14} className="text-green-400" />
                      <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Sales by Company</h2>
                    </div>
                    <div className="divide-y divide-navy-800/50">
                        ...
                        return (
                          <div key={c.slug} className="px-6 py-4 hover:bg-navy-800/30 transition-colors">
                            <div className="flex items-center gap-4 mb-2">
                              ...
                              <span className="text-xs text-navy-600 shrink-0">· {c.count} book{c.count !== 1 ? 's' : ''}</span>
                              <span className="text-[11px] text-navy-500 w-10 text-right shrink-0">{pct.toFixed(1)}%</span>
                            </div>
                            <div className="ml-9 h-1.5 bg-navy-800 rounded-full overflow-hidden">
              ...
              {sales.topSalePrice.length > 0 && (
                <div className="bg-navy-900 border border-navy-800 rounded-2xl overflow-hidden">
                  <div className="flex items-center gap-2 px-6 py-4 border-b border-navy-800">
                    ...
                    <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Top 10 Highest Sale Prices</h2>
                  </div>
                  <div className="divide-y divide-navy-800/50">
      ...
      {loadedTabs.has('pl') && (
        <div className={activeTab !== 'pl' ? 'hidden' : 'space-y-6'}>
          {salesLoading ? <TabLoading /> : !sales ? (
            ...
            <div className="text-center py-20 text-navy-500">No sales data yet — sell some books to see P&L analysis.</div>
          ) : (
            <>
              {/* Summary */}
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  ...
                  return (
                    <div className={`rounded-2xl p-5 border flex flex-col gap-2 ${totalPL >= 0 ? 'bg-emerald-950/20 border-emerald-700/30' : 'bg-red-950/20 border-red-700/30'}`}>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs uppercase tracking-wider text-navy-500">Overall P&amp;L</span>
                      </div>
              ...
              {sales.plByMonth && sales.plByMonth.length > 1 && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  <div className="bg-navy-900 border border-navy-800 rounded-2xl p-6 space-y-4">
                    <div className="flex items-center gap-2">
                      <TrendingUp size={14} className="text-emerald-400" />
                      <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Cumulative P&amp;L</h2>
                    </div>
                  ...
                  <div className="bg-navy-900 border border-navy-800 rounded-2xl p-6 space-y-4">
                    <div className="flex items-center gap-2">
                      ...
                      <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Monthly P&amp;L</h2>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  <div className="bg-navy-900 border border-navy-800 rounded-2xl p-6 space-y-4">
                    <div className="flex items-center gap-2">
                      ...
                      <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">ROI Distribution</h2>
                      <span className="ml-auto text-[10px] text-navy-600">{sales.salesWithROI.length} sales</span>
                    </div>
                  ...
                  <div className="bg-navy-900 border border-navy-800 rounded-2xl p-6 space-y-4">
                    <div className="flex items-center gap-2">
                      <Award size={14} className="text-brand-400" />
                      <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Hold Time vs ROI</h2>
                      <span className="ml-auto text-[10px] text-navy-600">days held → profit %</span>
                    </div>
              ...
              {sales.plByCompany && sales.plByCompany.length > 0 && (
                <div className="bg-navy-900 border border-navy-800 rounded-2xl p-6 space-y-4">
                  <div className="flex items-center gap-2">
                    <Tag size={14} className="text-brand-400" />
                    <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">P&amp;L by Publisher</h2>
                  </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {sales.topProfit.length > 0 && (
                  <div className="bg-navy-900 border border-navy-800 rounded-2xl overflow-hidden">
                    <div className="flex items-center gap-2 px-6 py-4 border-b border-navy-800">
                      <TrendingUp size={14} className="text-emerald-400" />
                      <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Top 10 Highest Profits</h2>
                    </div>
                    <div className="divide-y divide-navy-800/50">
                      {sales.topProfit.map((book, i) => (
                        <div key={i} className="flex items-center gap-4 px-6 py-3 hover:bg-navy-800/30 transition-colors">
                          ...
                          <div className="text-right shrink-0">
                            ...
                            <p className="text-xs text-navy-600">cost {fmt(book.cost, currency)}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {sales.topLoss.length > 0 && (
                  <div className="bg-navy-900 border border-navy-800 rounded-2xl overflow-hidden">
                    <div className="flex items-center gap-2 px-6 py-4 border-b border-navy-800">
                      <TrendingDown size={14} className="text-red-400" />
                      <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Top 10 Biggest Losses</h2>
                    </div>
                    <div className="divide-y divide-navy-800/50">
      ...
      {loadedTabs.has('features') && (
        <div className={activeTab !== 'features' ? 'hidden' : 'space-y-6'}>
          {featLoading ? <TabLoading /> : !features ? (
            ...
            <div className="text-center py-20 text-navy-500">No feature data yet.</div>
          ) : (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {Object.entries(features.byGroup).map(([group, items]) => (
                  <div key={group} className="bg-navy-900 border border-navy-800 rounded-2xl p-5 space-y-3">
                    <h3 className="text-xs uppercase tracking-wider text-brand-500 font-semibold capitalize">{group}</h3>
                    {items.map(item => (
                      <div key={item.slug} className="space-y-1">
                        <div className="flex justify-between text-xs">
                          <span className="text-navy-300 truncate">{item.label}</span>
                          <span className="text-navy-500 ml-2 shrink-0">{item.count} ({item.percent}%)</span>
                        </div>
                        <div className="h-1 bg-navy-800 rounded-full overflow-hidden">
            ...
            return (
              <>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  ...
                  <StatCard label="Unread" value={String(rc.unreadCount)} sub={`${rc.unreadPercent}% of owned`} icon={BookOpen} color="text-navy-400" />
                ...
                {total > 0 && (
                  <div className="bg-navy-900 border border-navy-800 rounded-2xl p-6 space-y-3">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Reading Progress</h2>
                    </div>
                    ...
                    <div className="flex gap-4 text-[11px] text-navy-500 flex-wrap">
                      ...
                      <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-navy-800 ring-1 ring-navy-600 inline-block" />Unread</span>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  {rc.readingBySubscription?.length > 0 && (
                    <div className="bg-navy-900 border border-navy-800 rounded-2xl overflow-hidden">
                      <div className="flex items-center gap-2 px-6 py-4 border-b border-navy-800">
                        <Tag size={14} className="text-brand-400" />
                        <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Reading by Subscription</h2>
                      </div>
                      <div className="divide-y divide-navy-800/50">
                        {rc.readingBySubscription.map((s) => {
                          const t = s.read + s.reading + s.unread + s.dnf
                          return (
                            <div key={s.slug} className="px-5 py-3.5 hover:bg-navy-800/30 transition-colors space-y-2">
                              <div className="flex items-center justify-between">
                                <span className="text-sm font-medium text-navy-200 truncate">{s.name}</span>
                                <span className="text-xs text-navy-500 ml-2 shrink-0">{t} books</span>
                              </div>
                  ...
                  {rc.readingByCompany?.length > 0 && (
                    <div className="bg-navy-900 border border-navy-800 rounded-2xl overflow-hidden">
                      <div className="flex items-center gap-2 px-6 py-4 border-b border-navy-800">
                        <Layers size={14} className="text-brand-400" />
                        <h2 className="text-sm font-semibold text-navy-300 uppercase tracking-wider">Reading by Company</h2>
                      </div>
                      <div className="divide-y divide-navy-800/50">
                        {rc.readingByCompany.slice(0, 10).map((c) => {
                          ...
                          return (
                            <div key={c.slug} className="px-5 py-3.5 hover:bg-navy-800/30 transition-colors space-y-2">
                              <div className="flex items-center justify-between">
                                <span className="text-sm font-medium text-navy-200 truncate">{c.name}</span>
                                <span className="text-xs text-navy-500 ml-2 shrink-0">{t} books</span>
                              </div>
                              <div className="h-2 bg-navy-800 rounded-full overflow-hidden flex">
