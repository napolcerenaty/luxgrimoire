    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-serif text-2xl font-bold text-navy-100">Data Requests</h1>
            <p className="text-sm text-navy-500">{data?.total ?? 0} total requests</p>
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 text-navy-500">No requests found.</div>
      ) : (
            ...
            <div key={r.id} className="bg-navy-900 border border-navy-800 rounded-2xl p-4 hover:border-navy-700 transition-colors">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="font-medium text-navy-200 text-sm">{r.name}</p>
                  {r.description && <p className="text-sm text-navy-400 mt-1 whitespace-pre-wrap">{r.description}</p>}
                  {r.referenceUrl && (
