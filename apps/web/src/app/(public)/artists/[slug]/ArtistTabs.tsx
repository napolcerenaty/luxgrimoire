  ...
  return (
    <div className="flex items-center justify-center gap-2 mt-8">
      <button
        ...
        disabled={page <= 1}
        className="p-1.5 rounded-lg border border-stone-700 text-stone-400 hover:text-amber-400 hover:border-amber-700 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
      >
        <ChevronLeft size={16} />
      </button>
      <span className="text-sm text-stone-400 px-2">
        {page} / {totalPages}
      </span>
      <button
        onClick={() => onPage(page + 1)}
...
function EditionGrid({ artistSlug }: { artistSlug: string }) {
  ...
  const { data: res, isLoading } = useQuery<PagedResponse<GroupedEdition>>({
  ...
  return (
    <>
      ...
      <Pagination page={res.page} totalPages={res.totalPages} onPage={setPage} />
    </>
  )
}
...
function CardMonthGrid({ artistSlug }: { artistSlug: string }) {
  ...
  const { data: res, isLoading } = useQuery<PagedResponse<CardMonth>>({
