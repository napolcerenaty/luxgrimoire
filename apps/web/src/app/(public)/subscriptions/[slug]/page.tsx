  ...
  return (
    <div className="container mx-auto px-4 py-10 max-w-5xl">
      ...
      {from === 'subscriptions' ? (
        <Link
          href="/subscriptions"
          className="flex items-center gap-2 mb-6 text-sm text-navy-400 hover:text-brand-400 transition-colors w-fit"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_260px] gap-8 mb-6 items-start">
        <div>
          <div className="flex items-center gap-3 flex-wrap mb-3">
            ...
            {sub.startDate && (
              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border border-navy-600 text-navy-400">
                Since {sub.startDate.slice(0, 7)}
              </span>
            )}
            {sub.endDate && (
              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border border-navy-600 text-navy-500">
                Ended {sub.endDate.slice(0, 7)}
              </span>
            )}
            {(sub.company as unknown as { hasOfficialImagePermission?: boolean })?.hasOfficialImagePermission && (
              <Link href="/companies-permissions">
                <Badge variant="outline">✓ Images used with brand permission</Badge>
              </Link>
            )}
          </div>

          <h1 className="text-4xl font-serif font-bold text-navy-100 mb-4 leading-tight">
            {sub.company?.slug && (
              <Link
                href={`/companies/${sub.company.slug}`}
                className="block text-sm font-sans font-normal text-navy-400 hover:text-brand-400 transition-colors mb-1"
              >
                {sub.company.name}
              </Link>
            )}
          ...
          {sub.description && (
            <p className="text-navy-300 leading-relaxed whitespace-pre-line">{sub.description}</p>
          )}
        ...
        comboFeatured.length > 0 && (
          <section className="mb-12 space-y-8">
                  ...
                  {comboFeatured
                    ...
                    .map(({ component, currentMonth: cur, currentSkip: curSkip }) => (
                      <div key={component.id} className="space-y-2">
                        <p className="text-xs font-semibold uppercase tracking-widest text-navy-400">{component.name}</p>
                        {curSkip ? (
            ...
            {comboFeatured.some((f) => f.upcomingMonth || f.upcomingSkip) && (
              <div>
                <h3 className="text-sm font-semibold uppercase tracking-widest text-navy-400 mb-4">Upcoming Theme</h3>
...
function FeaturedMonthCard({ label, labelVariant, monthData, accentColors, compact, skipped }: FeaturedMonthCardProps) {
  ...
  if (skipped) {
    return (
      <div className="rounded-2xl overflow-hidden bg-navy-900 border border-brand-800/40">
          ...
          <div className="absolute top-3 left-3">
            <span
              className={`text-xs font-semibold font-serif uppercase tracking-wider px-3 py-1 rounded-full ${
                labelVariant === 'current' ? 'bg-brand-500 text-navy-950' : 'bg-navy-700 text-brand-400 border border-brand-700/50'
              }`}
            >
              {label}
            </span>
        ...
        <div className={compact ? 'p-3' : 'p-5'}>
          <p className={`text-navy-100 font-serif font-bold mb-1 ${compact ? 'text-sm' : 'text-lg'}`}>
  ...
  const imageArea = (
    <div className={`group relative overflow-hidden bg-navy-950 cursor-pointer ${compact ? 'aspect-[16/9]' : 'aspect-[4/3]'}`}>
      {coverUrl ? (
        <>
      ...
      <div className="absolute inset-0 z-30 flex items-center justify-center bg-navy-950/65 px-4 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
        {mainBook?.edition ? (
          <p className="text-navy-100 text-sm font-serif font-semibold text-center leading-snug line-clamp-4">
            {mainBook.book.title}
          </p>
        ) : (
        ...
        <span
          className={`text-xs font-semibold font-serif uppercase tracking-wider px-3 py-1 rounded-full ${
            labelVariant === 'current'
              ...
              : 'bg-navy-700 text-brand-400 border border-brand-700/50'
          }`}
        >
          {label}
        </span>
  ...
  return (
    <div className="rounded-2xl overflow-hidden bg-navy-900 border border-navy-800 hover:border-brand-700/50 transition-colors">
      {mainBook ? (
      ...
      <div className={compact ? 'p-3' : 'p-5'}>
        ...
        {monthData.theme ? (
          <p className={`text-navy-200 font-serif italic ${compact ? 'text-xs mb-2' : 'text-sm mb-3'}`}>{monthData.theme}</p>
        ) : (
          <p className={`text-navy-500 italic ${compact ? 'text-xs mb-2' : 'text-sm mb-3'}`}>Theme not announced yet</p>
        )}

        {monthData.cardArtist && (
          <Link
            href={`/artists/${monthData.cardArtist.slug}`}
            className="inline-block text-xs text-navy-500 hover:text-brand-400 transition-colors mb-3"
          >
