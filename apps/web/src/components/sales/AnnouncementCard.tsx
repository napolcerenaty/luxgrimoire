...
export function AnnouncementCard({ a }: { a: ListSaleAnnouncement }) {
  ...
  return (
    <div title={a.title} className="relative group flex flex-col rounded-2xl bg-navy-900 border border-navy-800 hover:border-brand-700/60 transition-all hover:shadow-xl hover:shadow-brand-900/10">
      ...
      <div className="relative aspect-[2/3] bg-navy-950 overflow-hidden rounded-t-2xl">
        ...
        {a.isBundle && (
          <span className="absolute top-2 left-2 text-[9px] font-serif uppercase tracking-wider px-1.5 py-0.5 rounded bg-navy-950/80 border border-navy-600 text-brand-400">
            Bundle
          </span>
        )}
        {a.isSoldOut ? (
      ...
      <div className="p-3 flex flex-col flex-1">
        <div className="flex-1 flex flex-col gap-1">
          <p className="font-serif font-semibold text-navy-100 text-sm leading-snug line-clamp-2 group-hover:text-brand-400 transition-colors">
            {a.title}
          </p>
          {saleDate && <p className="text-xs text-brand-500">🗓 {saleDate}</p>}
          {a.basePrice != null && a.currency && (
            <p className="text-xs text-navy-400">from {a.basePrice} {a.currency}</p>
          )}
...
export function AnnouncementListRow({ a }: { a: ListSaleAnnouncement }) {
  ...
  return (
    ...
    >
      {/* Thumbnail */}
      <div className="w-12 h-12 shrink-0 rounded-lg overflow-hidden bg-navy-800 flex items-center justify-center relative">
        {thumb ? (
          // eslint-disable-next-line @next/next/no-img-element
          ...
          <>
            ...
            <p className="relative z-10 font-serif text-center text-[10px] leading-tight px-1 line-clamp-3 text-navy-300">
              {a.title}
            </p>
          </>
        )}
      ...
      <div className="flex-1 min-w-0">
        <p className="font-medium text-navy-100 group-hover:text-brand-400 transition-colors truncate leading-tight text-sm">
          {a.title}
        </p>
        <div className="flex items-center gap-2 flex-wrap mt-0.5">
          {a.company?.name && <span className="text-xs text-brand-600/80">{a.company.name}</span>}
          {saleDate && <span className="text-xs text-navy-400">🗓 {saleDate}</span>}
          {a.basePrice != null && a.currency && (
            <span className="text-xs text-navy-500">from {a.basePrice} {a.currency}</span>
          )}
          {a.subscriberBasePrice != null && (
      ...
      <div className="flex flex-col items-end gap-1 shrink-0">
        ...
        {a.isBundle && (
          <span className="text-[9px] font-serif uppercase tracking-wider px-1.5 py-0.5 rounded bg-navy-800 border border-navy-600 text-brand-400">Bundle</span>
        )}
      </div>
    </Link>
  )
