...
function MultiImageUpload({ images, folder, onChange }: {
  ...
  return (
    <div>
      ...
      <input ref={el => { inputRef.current = el }} type="file" accept="image/*" multiple className="hidden" onChange={handleFiles} />
      {images.length > 0 && (
...
export default function EditBookEditionForm({ edition, onSuccess, onCancel }: EditBookEditionFormProps) {
  ...
  const [publishYear, setPublishYear] = useState(edition.publishYear != null ? String(edition.publishYear) : '')
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input type="number" value={publishYear} onChange={e => setPublishYear(e.target.value)}
            min={1900} max={2100} className={INP} />
        </div>
      </div>
