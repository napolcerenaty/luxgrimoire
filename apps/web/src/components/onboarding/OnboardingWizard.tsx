...
export function OnboardingWizard() {
  ...
  // ── INPUT / LABEL classes ─────────────────────────────────────────────────
  const INPUT = 'w-full bg-navy-800 border border-navy-700 text-navy-100 rounded-lg px-3 py-2 text-sm placeholder:text-navy-500 focus:outline-none focus:border-brand-400 transition-colors'
  ...
  const steps = [
    ...
    <div key="prefs" className="space-y-4">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h2 className="font-serif text-xl font-semibold text-navy-100">Your preferences</h2>
          <p className="text-xs text-navy-500">Used for spending stats, fees and renewal dates</p>
        </div>
      </div>
      <div>
      ...
      <div>
        ...
        <p className="text-xs text-navy-500 mt-1">Used as default when adding subscription shipping costs</p>
      </div>
      <div>
        ...
        <p className="text-xs text-navy-500 mt-1">Used for skip deadlines and renewal date display</p>
      </div>
      <div>
          ...
          {([['24h', '24-hour (14:30)'], ['12h', '12-hour (2:30 PM)']] as [string, string][]).map(([val, desc]) => (
            <label key={val} className={`flex-1 flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${
              prefsTimeFormat === val
                ...
                : 'border-navy-700 bg-navy-800/50 text-navy-400 hover:border-navy-600'
            }`}>
              <input type="radio" name="onb-timeFormat" value={val} checked={prefsTimeFormat === val}
                onChange={() => setPrefsTimeFormat(val)} className="accent-brand-400" />
              <div>
    ...
    <div key="fees" className="space-y-4">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h2 className="font-serif text-xl font-semibold text-navy-100">Taxes & Fees</h2>
          <p className="text-xs text-navy-500">Reusable templates for VAT, customs, processing fees</p>
        </div>
      </div>
      {feesLoading ? (
        <div className="flex items-center gap-2 text-sm text-navy-400 py-2"><Loader2 size={14} className="animate-spin" /> Loading…</div>
      ) : existingTemplates.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs text-navy-400 font-medium uppercase tracking-wide">Your templates</p>
          {existingTemplates.map(t => (
            <div key={t.id} className="flex items-center justify-between bg-navy-800/50 border border-navy-700 rounded-lg px-3 py-2">
              <div>
                <span className="text-sm text-navy-200 font-medium">{t.name}</span>
                {t.category && <span className="ml-2 text-xs text-navy-500">{t.category}</span>}
              </div>
              {t.defaultAmount != null && (
                <span className="text-sm text-brand-400">{t.defaultAmount} {t.defaultCurrency ?? prefsCurrency}</span>
              )}
            </div>
          ))}
        </div>
      )}
      <div className="space-y-3 bg-navy-800/50 border border-navy-700 rounded-xl p-4">
        ...
        <button onClick={addFeeTemplate} disabled={feeSaving || !feeForm.name.trim()}
          className="flex items-center gap-2 text-sm font-medium text-brand-400 border border-brand-500/40 hover:border-brand-400 hover:bg-brand-500/10 disabled:opacity-40 disabled:cursor-not-allowed px-4 py-2 rounded-lg transition-colors">
          {feeSaving ? <><Loader2 size={13} className="animate-spin" /> Saving…</> : <><Plus size={13} /> Add template</>}
        </button>
      </div>
      <p className="text-xs text-navy-500">You can manage templates any time in <strong className="text-navy-400">Profile → Taxes & Fees</strong>.</p>
    </div>,
    ...
    <div key="notif" className="space-y-4">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h2 className="font-serif text-xl font-semibold text-navy-100">Notification Preferences</h2>
          <p className="text-xs text-navy-500">Get reminded about renewals and sales</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {pushSupported && permission !== 'denied' && (
          <div className="bg-navy-800/50 border border-navy-700 rounded-xl p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-navy-200">Enable push notifications</p>
                <p className="text-xs text-navy-500">Receive reminders even when the app is closed</p>
              </div>
              <button
                type="button"
                role="switch"
                ...
                className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors disabled:opacity-40 ${isSubscribed ? 'bg-brand-500' : 'bg-navy-700'}`}
              >
        ...
        <div className="bg-navy-800/50 border border-navy-700 rounded-xl p-4 space-y-3">
          <div>
            <p className="text-sm font-medium text-navy-200">App notifications</p>
            <p className="text-xs text-navy-500">Updates, bug fixes and announcements from the team</p>
          </div>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 opacity-60 select-none">
              <div className="relative inline-flex h-5 w-9 shrink-0 rounded-full bg-brand-500 cursor-not-allowed">
                <span className="inline-block h-4 w-4 transform rounded-full bg-white shadow translate-x-4 mt-0.5" />
              </div>
              <span className="text-xs text-navy-300">In-app (always on)</span>
            </div>
            {pushSupported && isSubscribed && (
              <label className="flex items-center gap-2 cursor-pointer select-none">
                <button type="button" role="switch" aria-checked={notifAppPush}
                  ...
                  className={`relative inline-flex h-5 w-9 shrink-0 rounded-full border-2 border-transparent transition-colors ${notifAppPush ? 'bg-brand-500' : 'bg-navy-700'}`}>
                ...
                <span className="text-xs text-navy-300">Push</span>
              </label>
            )}
          </div>
        </div>
        ...
        <div className="bg-navy-800/50 border border-navy-700 rounded-xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-navy-200">Renewal reminders</p>
              <p className="text-xs text-navy-500">Get notified before your subscriptions renew</p>
            </div>
            <button
              type="button"
              role="switch"
              ...
              className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors ${notifRenewalEnabled ? 'bg-brand-500' : 'bg-navy-700'}`}
            >
          ...
          {notifRenewalEnabled && (
            <div className="space-y-3 pt-1 border-t border-navy-700/50">
              {/* Channels */}
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  <button type="button" role="switch" aria-checked={notifRenewalInApp}
                    onClick={() => setNotifRenewalInApp(v => !v)}
                    className={`relative inline-flex h-5 w-9 shrink-0 rounded-full border-2 border-transparent transition-colors ${notifRenewalInApp ? 'bg-brand-500' : 'bg-navy-700'}`}>
                  ...
                  <span className="text-xs text-navy-300">In-app</span>
                </label>
                {pushSupported && isSubscribed && (
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <button type="button" role="switch" aria-checked={notifRenewalPush}
                      onClick={() => setNotifRenewalPush(v => !v)}
                      className={`relative inline-flex h-5 w-9 shrink-0 rounded-full border-2 border-transparent transition-colors ${notifRenewalPush ? 'bg-brand-500' : 'bg-navy-700'}`}>
        ...
        <div className="bg-navy-800/50 border border-navy-700 rounded-xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-navy-200">Sale reminders</p>
              <p className="text-xs text-navy-500">Get reminded about sales you're interested in</p>
            </div>
            <button
              type="button"
              role="switch"
              ...
              className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors ${notifSaleEnabled ? 'bg-brand-500' : 'bg-navy-700'}`}
            >
              ...
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  <button type="button" role="switch" aria-checked={notifSaleInApp}
                    ...
                    className={`relative inline-flex h-5 w-9 shrink-0 rounded-full border-2 border-transparent transition-colors ${notifSaleInApp ? 'bg-brand-500' : 'bg-navy-700'}`}>
                ...
                {pushSupported && isSubscribed && (
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <button type="button" role="switch" aria-checked={notifSalePush}
                      ...
                      className={`relative inline-flex h-5 w-9 shrink-0 rounded-full border-2 border-transparent transition-colors ${notifSalePush ? 'bg-brand-500' : 'bg-navy-700'}`}>
      ...
      <p className="text-xs text-navy-500">You can adjust all notification settings any time in <strong className="text-navy-400">Profile → Notifications</strong>.</p>
    </div>,
    ...
    <div key="footer" className="space-y-4">
      <div className="flex items-center gap-3 mb-2">
        ...
        <h2 className="font-serif text-xl font-semibold text-navy-100">Footer links</h2>
      </div>
      <p className="text-navy-400 text-sm">In the footer you&apos;ll find several useful links:</p>
      <div className="space-y-3">
        <div className="bg-navy-800/50 border border-navy-700 rounded-xl p-3 space-y-1">
          <p className="text-sm font-medium text-navy-200">🐛 Bug Report</p>
          <p className="text-xs text-navy-400">Found something broken? Submit a bug report and we&apos;ll look into it.</p>
        </div>
        <div className="bg-navy-800/50 border border-navy-700 rounded-xl p-3 space-y-1">
          <p className="text-sm font-medium text-navy-200">💡 Feature Request</p>
          <p className="text-xs text-navy-400">Have an idea for a new feature? Tell us — we read every request.</p>
        </div>
        <div className="bg-navy-800/50 border border-navy-700 rounded-xl p-3 space-y-1">
          <p className="text-sm font-medium text-navy-200">📦 Sale Request</p>
          <p className="text-xs text-navy-400">Know about an upcoming sale or promotion? Submit it so the community can see it.</p>
        </div>
        <div className="bg-navy-800/50 border border-navy-700 rounded-xl p-3 space-y-1">
          <p className="text-sm font-medium text-navy-200">📋 Request Data</p>
          <p className="text-xs text-navy-400">Missing a book, edition, or other data in the system? Submit a data request and we&apos;ll add it.</p>
        </div>
      </div>
    </div>,
    ...
    <div key="photos" className="space-y-4">
      <div className="flex items-center gap-3 mb-2">
        ...
        <h2 className="font-serif text-xl font-semibold text-navy-100">Community Photos</h2>
      </div>
      ...
      <div className="space-y-3">
        {[
          ...
          <div key={i} className="flex items-start gap-3">
            <span className="w-6 h-6 rounded-full bg-brand-500/20 text-brand-400 text-xs flex items-center justify-center font-semibold shrink-0 mt-0.5">{i + 1}</span>
    ...
    <div key="subs" className="space-y-4">
      <div className="flex items-center gap-3 mb-2">
        ...
        <h2 className="font-serif text-xl font-semibold text-navy-100">Tracking Subscriptions</h2>
      </div>
      <p className="text-navy-400 text-sm leading-relaxed">
        LuxGrimoire can backfill your entire subscription history — you don&apos;t need to add each box manually.
      </p>
      <div className="bg-navy-800/50 border border-brand-500/20 rounded-xl p-4 space-y-3">
        <p className="text-sm font-medium text-brand-300 flex items-center gap-2"><Sparkles size={14} /> Here&apos;s what happens automatically:</p>
        <ul className="space-y-2 text-sm text-navy-300">
          <li className="flex items-start gap-2"><span className="text-brand-400 mt-0.5">→</span><span><strong>Set the start date</strong> of your subscription</span></li>
          <li className="flex items-start gap-2"><span className="text-brand-400 mt-0.5">→</span><span><strong>Enter your costs</strong> (price, shipping, fees)</span></li>
          <li className="flex items-start gap-2"><span className="text-brand-400 mt-0.5">→</span><span>We <strong>calculate all past boxes</strong>, skipped months, and your next renewal date</span></li>
          <li className="flex items-start gap-2"><span className="text-brand-400 mt-0.5">→</span><span>Books are <strong>automatically linked</strong> to the correct months</span></li>
        </ul>
      </div>
      <p className="text-xs text-navy-500">You can always adjust individual months or change costs after the fact.</p>
    </div>,
  ]

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-navy-950 border border-navy-800 rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="flex items-center justify-between px-6 pt-5 pb-3 border-b border-navy-800 shrink-0">
          <div className="flex items-center gap-2">
            <Sparkles size={16} className="text-brand-400" />
            <span className="text-sm font-medium text-navy-300">Getting started</span>
          </div>
          <button onClick={() => completeOnboarding(false)} className="text-navy-500 hover:text-navy-300 transition-colors" title="Skip tutorial">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex items-center gap-1.5 px-6 py-3 shrink-0">
          {Array.from({ length: TOTAL_STEPS }).map((_, i) => (
            <div key={i} className={`h-1 rounded-full transition-all ${i <= step ? 'bg-brand-400' : 'bg-navy-700'} ${i === step ? 'flex-[2]' : 'flex-1'}`} />
          ))}
        </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-navy-800 shrink-0">
          <button
            ...
            className="flex items-center gap-1 text-sm text-navy-400 hover:text-navy-200 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
          >
          ...
          {step < TOTAL_STEPS - 1 ? (
            <button
              ...
              className="flex items-center gap-2 bg-brand-500 hover:bg-brand-400 disabled:opacity-60 text-navy-950 font-semibold text-sm px-5 py-2 rounded-xl transition-colors"
            >
              {(prefsSaving || notifSaving) ? <><Loader2 size={14} className="animate-spin" /> Saving…</> : <>Next <ChevronRight size={16} /></>}
            </button>
          ) : (
            <div className="flex items-center gap-2">
              <button onClick={() => completeOnboarding(false)} className="text-sm text-navy-400 hover:text-navy-200 px-4 py-2 rounded-xl transition-colors">
                Done
              </button>
              <button
                onClick={() => completeOnboarding(true)}
                className="flex items-center gap-2 bg-brand-500 hover:bg-brand-400 text-navy-950 font-semibold text-sm px-5 py-2 rounded-xl transition-colors"
              >
