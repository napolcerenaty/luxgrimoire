...
export function CancelSubscriptionModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-navy-900 border border-navy-700 rounded-2xl w-full max-w-sm shadow-2xl">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <label className="block text-xs text-navy-400 mb-1">Cancellation date</label>
            <input
              type="date"
              value={cancellationDate}
              onChange={e => handleDateChange(e.target.value)}
              className={`w-full bg-navy-800 border rounded-lg px-3 py-2 text-navy-100 text-sm ${dateInvalid ? 'border-red-500/70' : 'border-navy-700'}`}
            />
          </div>

          <div>
            <label className="block text-xs text-navy-400 mb-1">Reason (optional)</label>
            <textarea
              value={reason}
              onChange={e => setReason(e.target.value)}
              rows={3}
              placeholder="e.g. Too expensive, changed box, etc."
              className="w-full bg-navy-800 border border-navy-700 rounded-lg px-3 py-2 text-navy-100 text-sm resize-none"
            />
          </div>
        ...
        <div className="flex gap-3 p-5 border-t border-navy-800">
          ...
          <button type="button" onClick={handleConfirm} disabled={saving}
            className="flex-1 py-2 rounded-lg bg-red-800 hover:bg-red-700 text-navy-100 text-sm font-medium transition-colors disabled:opacity-50">
