...
export default function WaitlistPanel() {
  ...
  if (isLoading) {
    return (
      <div className="bg-navy-900 border border-navy-800 rounded-2xl p-6">
        <h2 className="font-serif font-semibold text-navy-100 mb-4 flex items-center gap-2">
  ...
  return (
    <div className="bg-navy-900 border border-navy-800 rounded-2xl p-6 space-y-4">
      <h2 className="font-serif font-semibold text-navy-100 flex items-center gap-2">
      ...
      {active.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-medium text-navy-400 uppercase tracking-wider">Currently Waiting</p>
          {active.map((entry) => (
            <WaitlistRow
              key={entry.id}
              entry={entry}
      ...
      {history.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-medium text-navy-400 uppercase tracking-wider flex items-center gap-1.5">
...
function WaitlistRow({
  ...
  return (
    <div className="flex items-center gap-3 bg-navy-800/60 border border-navy-700/50 rounded-xl px-4 py-3 group">
      {logoUrl ? (
        <Image src={logoUrl} alt={sub.company?.name ?? ''} width={32} height={32} className="w-8 h-8 rounded-lg object-cover shrink-0" unoptimized />
      ) : (
        <div className="w-8 h-8 rounded-lg bg-navy-700 shrink-0" />
      )}

      <div className="flex-1 min-w-0">
        <Link
          href={`/subscriptions/${sub.slug}`}
          className="text-sm font-medium text-navy-100 hover:text-brand-400 transition-colors line-clamp-1"
        >
          {sub.name}
        </Link>
      ...
      <button
        onClick={onLeave}
        disabled={leaving}
        className="opacity-0 group-hover:opacity-100 flex items-center gap-1 text-xs text-navy-500 hover:text-red-400 transition-all disabled:opacity-50 shrink-0"
...
function WaitlistHistoryRow({ entry }: { entry: ApiWaitlistEntry }) {
  ...
  return (
    <div className="flex items-center gap-3 bg-navy-800/30 border border-navy-800 rounded-xl px-4 py-3 opacity-70">
      {logoUrl ? (
        <Image src={logoUrl} alt={sub.company?.name ?? ''} width={32} height={32} className="w-8 h-8 rounded-lg object-cover shrink-0" unoptimized />
      ) : (
        <div className="w-8 h-8 rounded-lg bg-navy-700 shrink-0" />
      ...
      <div className="flex-1 min-w-0">
        <Link
          href={`/subscriptions/${sub.slug}`}
          className="text-sm font-medium text-navy-300 hover:text-brand-400 transition-colors line-clamp-1"
        >
          {sub.name}
        </Link>
