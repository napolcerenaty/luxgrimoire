...
export function renewalDayInMonth(entry: CalEntry, year: number, month0: number): number | null {
  ...
  // If that box month is skipped — either the user's own skip, or the subscription's own company-wide
  // "this month doesn't ship" (sub.monthSkips) — no renewal fires for this calendar month.
