-- Apply all columns that were missed due to failed migration cycles.
-- Fully idempotent — safe to run on any environment.

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_book_entries' AND column_name = 'order_number') THEN
    ALTER TABLE "user_book_entries" ADD COLUMN "order_number" VARCHAR(255);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'sale_announcements' AND column_name = 'subscriber_base_price') THEN
    ALTER TABLE "sale_announcements" ADD COLUMN "subscriber_base_price" DECIMAL(10,2) NULL;
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'sale_announcement_regions' AND column_name = 'subscriber_base_price') THEN
    ALTER TABLE "sale_announcement_regions" ADD COLUMN "subscriber_base_price" DECIMAL(10,2) NULL;
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_sale_interests' AND column_name = 'selected_price') THEN
    ALTER TABLE "user_sale_interests" ADD COLUMN "selected_price" DECIMAL(10,2) NULL;
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_sale_interests' AND column_name = 'selected_price_currency') THEN
