...
CREATE INDEX CONCURRENTLY IF NOT EXISTS "book_series_name_trgm_gin"
  ON books USING gin ("seriesName" gin_trgm_ops)
  WHERE "seriesName" IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS "authors_name_trgm_gin"
  ON authors USING gin (name gin_trgm_ops);

CREATE INDEX CONCURRENTLY IF NOT EXISTS "artists_name_trgm_gin"
  ON artists USING gin (name gin_trgm_ops);

CREATE INDEX CONCURRENTLY IF NOT EXISTS "subscriptions_name_trgm_gin"
  ON subscriptions USING gin (name gin_trgm_ops);

CREATE INDEX CONCURRENTLY IF NOT EXISTS "book_box_companies_name_trgm_gin"
  ON book_box_companies USING gin (name gin_trgm_ops);

CREATE INDEX CONCURRENTLY IF NOT EXISTS "sale_announcements_title_trgm_gin"
  ON sale_announcements USING gin (title gin_trgm_ops)
  WHERE title IS NOT NULL;

-- Also covers publisher field searched in editions fallback
CREATE INDEX CONCURRENTLY IF NOT EXISTS "book_editions_publisher_trgm_gin"
